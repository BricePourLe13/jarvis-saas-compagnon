...
-- Index pour performance gym_members
...
-- Index pour jarvis_conversation_logs
CREATE INDEX IF NOT EXISTS idx_jarvis_logs_session_turn ON jarvis_conversation_logs(session_id, conversation_turn_number);
CREATE INDEX IF NOT EXISTS idx_jarvis_logs_member_time ON jarvis_conversation_logs(member_id, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_jarvis_logs_gym_time ON jarvis_conversation_logs(gym_id, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_jarvis_logs_intent ON jarvis_conversation_logs(detected_intent);
CREATE INDEX IF NOT EXISTS idx_jarvis_logs_review ON jarvis_conversation_logs(needs_human_review) WHERE needs_human_review = true;
...
-- Index pour member_knowledge_base
CREATE INDEX IF NOT EXISTS idx_knowledge_base_member_active ON member_knowledge_base(member_id, is_active);
CREATE INDEX IF NOT EXISTS idx_knowledge_base_type_confidence ON member_knowledge_base(knowledge_type, confidence_level DESC);
CREATE INDEX IF NOT EXISTS idx_knowledge_base_confirmed ON member_knowledge_base(last_confirmed_at DESC);
...
-- Index pour member_session_analytics
CREATE INDEX IF NOT EXISTS idx_session_analytics_member_date ON member_session_analytics(member_id, session_date DESC);
CREATE INDEX IF NOT EXISTS idx_session_analytics_date_engagement ON member_session_analytics(session_date, high_engagement);
CREATE INDEX IF NOT EXISTS idx_session_analytics_complaints ON member_session_analytics(contained_complaint) WHERE contained_complaint = true;
...
-- RLS pour session analytics
ALTER TABLE member_session_analytics ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS session_analytics_gym_access ON member_session_analytics;
CREATE POLICY session_analytics_gym_access ON member_session_analytics
  FOR ALL TO authenticated
  USING (
    member_id IN (
      SELECT gm.id FROM gym_members gm 
