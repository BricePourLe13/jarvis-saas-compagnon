...
BEGIN
    ...
    SELECT 
        ...
        END as status,
        CASE 
            WHEN s.ended_at IS NULL THEN EXTRACT(EPOCH FROM (now() - s.started_at))::INTEGER
        ...
        (s.metadata->>'member_name')::TEXT as member_name
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(*) FILTER (WHERE s.ended_at IS NULL)::INTEGER as active_sessions,
        ...
        MAX(s.started_at) as last_session_time
    FROM openai_realtime_sessions s
    WHERE s.gym_id = p_gym_id 
      AND s.started_at > (now() - (p_hours_back || ' hours')::INTERVAL);
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    WHERE s.gym_id = p_gym_id 
      AND s.ended_at IS NULL
      AND EXTRACT(EPOCH FROM (now() - s.started_at)) > 600;
...
SELECT 
    ...
    COUNT(s.id) FILTER (WHERE s.ended_at IS NULL) as active_sessions,
    COUNT(s.id) FILTER (WHERE s.started_at > now() - INTERVAL '24 hours') as sessions_24h,
    COALESCE(SUM(s.total_cost_usd) FILTER (WHERE s.started_at > now() - INTERVAL '24 hours'), 0) as cost_24h,
    COALESCE(AVG(s.session_duration_seconds) FILTER (WHERE s.started_at > now() - INTERVAL '24 hours'), 0) as avg_duration_24h,
    MAX(s.started_at) as last_session_time,
    -- Status global du kiosk
    CASE 
        WHEN COUNT(s.id) FILTER (WHERE s.ended_at IS NULL) > 0 THEN 'active'
        WHEN MAX(s.started_at) > now() - INTERVAL '2 hours' THEN 'recent'
        WHEN MAX(s.started_at) > now() - INTERVAL '24 hours' THEN 'idle'
        ELSE 'offline'
    END as kiosk_status
FROM gyms g
